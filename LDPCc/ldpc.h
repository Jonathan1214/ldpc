...
class LDPC {
 ...
 public:
  ...
  LDPC() = default;
  LDPC(int _len, int _bits)
      : len_(_len), bits_(_bits), code_ratio_(_bits / (_len + 0.0)) {}
  LDPC(const CodeConfigure& ccf);
  // initial code
  void initial(int _len, int _bits, int bx, int by, int _gf_1,
               const string& _cnsfile, const string& _vnsfile, char delim = ',',
               char corr = 1);
  inline void init_memo(const string& _cnsfile, const string& _vnsfile,
                        char delim, char corr);
  inline void init_memo(const string& _cnsfile, const string& _vnsfile,
  ...
  inline string get_H_size() const {
    return to_string(row_of_h_matrix_) + "x" + to_string(column_of_h_matrix_);
  }
