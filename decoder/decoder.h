...
class Decoder {
 public:
  Decoder() = default;
  Decoder(int _iteration, float _attenuation)
      : iteration(_iteration), attenuation(_attenuation) {}
  Decoder(const CodeConfigure &ccf);
  void initial(const LDPC &cc);
  void initial(const CodeConfigure &ccf);
  // use MSA by default
  int run_decoder(const LDPC &cc, const ReceivedSequencesType &sq, double ebn0,
                  DE_ALGO algo = msa);
 ...
 private:
  ...
  inline void get_llr(const ReceivedSequencesType &sq, double code_ratio,
                      double ebn0);
  void cn_update(const LDPC &cc);
  void vn_update(const LDPC &cc);
  inline int check(
      const LDPC &cc);  // return checksum of all row check of LDPC code
