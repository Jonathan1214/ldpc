...
#define LOG_DE(ebn0, tot_blk, err_blk, err_bits, type)                         \
  do {                                                                         \
    curtime = time(0);                                                         \
    logtime = localtime(&curtime);                                             \
    fprintf(stdout,                                                            \
            type                                                               \
            " [%02d-%02d-%02d %02d:%02d:%02d]  %6.2f  %10llu  %4d  %5d\n",     \
            logtime->tm_year + 1900, logtime->tm_mon + 1, logtime->tm_mday,    \
            logtime->tm_hour, logtime->tm_min, logtime->tm_sec, ebn0, tot_blk, \
            err_blk, err_bits);                                                \
    fflush(stdout);                                                            \
  } while (0)

#define LOG_DE_RATIO(ebn0, tot_blk, err_blk, err_bits, frame_len, type)        \
  do {                                                                         \
    curtime = time(0);                                                         \
    logtime = localtime(&curtime);                                             \
    fprintf(stdout,                                                            \
            ...
            logtime->tm_year + 1900, logtime->tm_mon + 1, logtime->tm_mday,    \
            logtime->tm_hour, logtime->tm_min, logtime->tm_sec, ebn0, tot_blk, \
            err_blk, err_bits, (double)err_blk / tot_blk,                      \
            (double)err_bits / tot_blk / frame_len);                           \
    fflush(stdout);                                                            \
  } while (0)

#define INITIAL_CODE(code)               \
  do {                                   \
    for (auto &c : code) c = rand() % 2; \
  } while (0)
...
#define HELP()                                                                 \
  cout << "This is a QC-LDPC simulator which show how well a code is\n"        \
       << endl;                                                                \
  cout << "Here is the configuration of the simulator:" << endl;               \
  cout << " --len             code length                     ex: 2048"        \
       << endl;                                                                \
  cout << " --row             row of H matrix                 ex: 1024"        \
       << endl;                                                                \
  cout << " --bits            info bits length                ex: 1027"        \
       << endl;                                                                \
  cout << " --cnfile          row connection file             ex: "            \
          "./matrix/CnsCon.csv"                                                \
       << endl;                                                                \
  cout << " --vnfile          col connection file             ex: "            \
          "./matrix/VnsCon.csv"                                                \
       << endl;                                                                \
  cout << " --cnweight        check node weght of code	       ex: 8" << endl; \
  cout << " --vnweight        variable node weight of code    ex: 4" << endl;  \
  cout << " --gfield          Galois Fiels size minus 1       ex: 256"         \
       << endl;                                                                \
  cout << " --sebno           start ebn0 of simulation        ex: 2.5"         \
       << endl;                                                                \
  cout << " --stepebno        step ebn0 of simulation         ex: 0.2"         \
       << endl;                                                                \
  cout << " --eebno           end  ebn0 of simulation         ex: 3.0"         \
       << endl;                                                                \
  cout << " [--file]          read configuration from file    ex: config "     \
          "!!!!!!!!! NOT USED"                                                 \
       << endl;                                                                \
  cout << " [--decoding]      decoding selection              default or "     \
          "0->msa 1->spa"                                                      \
       << endl;                                                                \
  cout << " [--iteration]     iteration of decoding           ex: 50" << endl; \
  cout << " [--attenuation]   attenuation of msa 	           ex: 0625 "        \
          "(due to a bug in command line input, only integer input is OK)"     \
       << endl;                                                                \
  cout << " [--encoding]      select encoding		           ex:"                 \
       << endl;                                                                \
  cout << " [--genematrix]    generation matrix file          ex: "            \
          "./matrix/G.csv"                                                     \
       << endl;                                                                \
  cout << " --help            get help information" << endl;                   \
  cout << endl;                                                                \
  cout << "Also, you can use short args" << endl;                              \
  cout << " -l                code length                     ex: 2048"        \
       << endl;                                                                \
  cout << " -r                row of H matrix                 ex: 1024"        \
       << endl;                                                                \
  cout << " -b                info bits length                ex: 1027"        \
       << endl;                                                                \
  cout << " -c                row connection file	           ex: "             \
          "./matrix/CnsCon.csv"                                                \
       << endl;                                                                \
  cout << " -v                col connection file             ex: "            \
          "./matrix/VnsCon.csv"                                                \
       << endl;                                                                \
  ...
  cout << " -s                start ebn0 of simulation        ex: 2.5"         \
       << endl;                                                                \
  cout << " -S                step ebn0 of simulation         ex: 0.2"         \
       << endl;                                                                \
  cout << " -E                end  ebn0 of simulation         ex: 3.0"         \
       << endl;                                                                \
  cout << " [-d]              decoding selection              default or "     \
          "0->msa 1->spa"                                                      \
       << endl;                                                                \
  cout << " [-i]              iteration of decoding           ex: 50" << endl; \
  cout << " [-a]              attenuation of msa 	           ex: 625 "         \
          "(due to a bug in command line input, only integer input is OK)"     \
       << endl;                                                                \
  cout << " [-f]              read configuration from file    ex: config "     \
          "!!!!!!!! NOT USE"                                                   \
       << endl;                                                                \
  cout << " [-e]              select encoding		           ex:"                 \
       << endl;                                                                \
  cout << " [-x]              generation matrix file          ex: "            \
          "./matrix/G.csv"                                                     \
       << endl;                                                                \
...
static inline void parse_arg(int argc, char *argv[]) {
  ...
  int o;
  while ((o = getopt_long(argc, argv, "ehl:r:b:c:v:C:V:G:d::i::a::x:s:S:E:",
                          table, NULL)) != -1) {
    switch (o) {
      case 'l':
