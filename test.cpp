...
#define print_array(T, size)                                    \
  {                                                             \
    for (int i = 0; i < size; ++i) cout << (int)T.at(i) << " "; \
    cout << endl;                                               \
  }
...
void test_code() {
  ...
  string case_in_file(
      "../../ldpc_gf257_multi_code_ratio/base4x8/matrix/case_in.csv");
  string case_out_file(
      "../../ldpc_gf257_multi_code_ratio/base4x8/matrix/case_out.csv");
  ...
  while (line < ac.get_len() && getline(fp_case_in, s)) {
    ...
    if (comp_equal(case_out, encoder.coded_output))
      ok_cnt++;
    else {
